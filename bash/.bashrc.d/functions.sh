...
# Pick a file with fzf, scp to destination
sff() {
  if [ $# -eq 0 ]; then
...
# nvim shortcut: bare `n` opens cwd
n() {
  if [ "$#" -eq 0 ]; then command nvim .; else command nvim "$@"; fi
}

# Switch between Neovim configs
nvims() {
  local items=("default" "LazyVim") config
  config=$(printf "%s\n" "${items[@]}" \
...
# tar -czf wrapper (decompress alias is in aliases.sh)
compress() { tar -czf "${1%/}.tar.gz" "${1%/}"; }

# ── omarchy CLI completions ─────────────────────────────────────────────
_omarchy_complete() {
  COMPREPLY=()
